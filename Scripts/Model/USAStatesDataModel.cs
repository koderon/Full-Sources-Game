...
public class USAStatesDataModel : IGameDataModel
{
    private List<IGameDataParticleModel> _data = new List<IGameDataParticleModel>();

    public void Init()
    {
        var text = FileUtils.LoadTextFromResources("db/states");
        ...
        for (int i = 1; i < statesData.Length; i++)
        {
            ...
            var state = new USAStateDataModel();
...
public class USAStateDataModel : IGameDataParticleModel
{
    public int Id;
    public string Level;

    private TypeDataModel _type = TypeDataModel.States;
